...
public class FileProvider extends ContentProvider {
    ...
    class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            try {
                ...
                return new Builder().scheme(RMsgInfo.COL_CONTENT).authority(this.mAuthority).encodedPath(Uri.encode((String) entry.getKey()) + '/' + Uri.encode(path2.endsWith("/") ? canonicalPath.substring(path2.length()) : canonicalPath.substring(path2.length() + 1), "/")).build();
            } catch (IOException e) {
    ...
    private static PathStrategy parsePathStrategy(Context context, String str) {
        ...
        XmlResourceParser loadXmlMetaData = context.getPackageManager().resolveContentProvider(str, SmileConstants.TOKEN_PREFIX_TINY_UNICODE).loadXmlMetaData(context.getPackageManager(), META_DATA_FILE_PROVIDER_PATHS);

...
public class ViewDragHelper {
    ...
    final class C01131 implements Interpolator {
        C01131() {
        }
    ...
    static {
        sInterpolator = new C01131();
    }

    private ViewDragHelper(Context context, ViewGroup viewGroup, Callback callback) {
        this.mActivePointerId = INVALID_POINTER;
        this.mSetIdleRunnable = new C01142();
        if (viewGroup == null) {
            ...
            this.mEdgeSize = (int) ((context.getResources().getDisplayMetrics().density * TitleBar.BACKBTN_LEFT_MARGIN) + 0.5f);
    ...
    private boolean checkTouchSlop(View view, float f, float f2) {
        ...
        boolean z = this.mCallback.getViewHorizontalDragRange(view) > 0 ? true : STATE_IDLE;
        boolean z2 = this.mCallback.getViewVerticalDragRange(view) > 0 ? true : STATE_IDLE;
    ...
    private int computeAxisDuration(int i, int i2, int i3) {
        ...
        float distanceInfluenceForSnapDuration = (distanceInfluenceForSnapDuration(Math.min(C2020f.f10933c, ((float) Math.abs(i)) / ((float) width))) * ((float) i4)) + ((float) i4);
        i4 = Math.abs(i2);
        return Math.min(i4 > 0 ? Math.round(Math.abs(distanceInfluenceForSnapDuration / ((float) i4)) * 1000.0f) * EDGE_TOP : (int) (((((float) Math.abs(i)) / ((float) i3)) + C2020f.f10933c) * 256.0f), MAX_SETTLE_DURATION);
    }
    ...
    public static ViewDragHelper create(ViewGroup viewGroup, float f, Callback callback) {
        ...
        create.mTouchSlop = (int) (((float) create.mTouchSlop) * (C2020f.f10933c / f));
        return create;
    }
    ...
    private void releaseViewForPointerUp() {
        this.mVelocityTracker.computeCurrentVelocity(XiaomiOAuthConstants.SCOPE_ACCESS_MI_ROUTER, this.mMaxVelocity);
    ...
    public boolean checkTouchSlop(int i, int i2) {
        ...
        boolean z = (i & STATE_DRAGGING) == STATE_DRAGGING ? true : STATE_IDLE;
        boolean z2 = (i & STATE_SETTLING) == STATE_SETTLING ? true : STATE_IDLE;

...
public final class SendAuth {
    ...
    public class Req extends BaseReq {
        ...
        final boolean checkArgs() {
            if (this.scope == null || this.scope.length() == 0 || this.scope.length() > SmileConstants.MAX_SHARED_STRING_VALUES) {
                Log.m13541e("MicroMsg.SDK.SendAuth.Req", "checkArgs fail, scope is invalid");
                return false;
            } else if (this.state == null || this.state.length() <= SmileConstants.MAX_SHARED_STRING_VALUES) {
                return true;
            } else {
                Log.m13541e("MicroMsg.SDK.SendAuth.Req", "checkArgs fail, state is invalid");
                return false;
            }
        }
    ...
    public class Resp extends BaseResp {
        ...
        final boolean checkArgs() {
            if (this.state == null || this.state.length() <= SmileConstants.MAX_SHARED_STRING_VALUES) {
                return true;
            }
            Log.m13541e("MicroMsg.SDK.SendAuth.Resp", "checkArgs fail, state is invalid");
            return false;
        }

...
public final class TrafficStats {
    ...
    public static void update() {
        ...
        try {
            ...
            if (bl < 0) {
                bl = j;
                Log.m13548v("MicroMsg.SDK.TrafficStats", "fix loss newMobileTx %d", Long.valueOf(j));
            }
            if (bm < 0) {
                bm = j2;
                Log.m13548v("MicroMsg.SDK.TrafficStats", "fix loss newMobileRx %d", Long.valueOf(j2));
            }
            if (bn < 0) {
                bn = j3;
                Log.m13548v("MicroMsg.SDK.TrafficStats", "fix loss newWifiTx %d", Long.valueOf(j3));
            }
            if (bo < 0) {
                bo = j4;
                Log.m13548v("MicroMsg.SDK.TrafficStats", "fix loss newWifiRx %d", Long.valueOf(j4));
            }
            if (j4 - bo < 0) {
                Log.m13548v("MicroMsg.SDK.TrafficStats", "minu %d", Long.valueOf(j4 - bo));
            }
            if (j3 - bn < 0) {
                Log.m13548v("MicroMsg.SDK.TrafficStats", "minu %d", Long.valueOf(j3 - bn));
            }
            bp = j >= bl ? j - bl : j;
            bq = j2 >= bm ? j2 - bm : j2;
            br = j3 >= bn ? j3 - bn : j3;
        ...
        Log.m13540d("MicroMsg.SDK.TrafficStats", "current system traffic: wifi rx/tx=%d/%d, mobile rx/tx=%d/%d", Long.valueOf(bs), Long.valueOf(br), Long.valueOf(bq), Long.valueOf(bp));
    }

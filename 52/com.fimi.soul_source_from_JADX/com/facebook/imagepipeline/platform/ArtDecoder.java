...
public class ArtDecoder implements PlatformDecoder {
    ...
    static {
        EOI_TAIL = new byte[]{(byte) -1, (byte) -39};
    }

    public ArtDecoder(BitmapPool bitmapPool, int i) {
        this.mBitmapPool = bitmapPool;
        this.mDecodeBuffers = new SynchronizedPool(i);
        for (int i2 = 0; i2 < i; i2++) {
            this.mDecodeBuffers.release(ByteBuffer.allocate(DECODE_BUFFER_SIZE));
        }
    }

    private CloseableReference<Bitmap> decodeStaticImageFromStream(InputStream inputStream, Options options) {
        ...
        ByteBuffer allocate = byteBuffer == null ? ByteBuffer.allocate(DECODE_BUFFER_SIZE) : byteBuffer;
        try {
    ...
    public CloseableReference<Bitmap> decodeJPEGFromEncodedImage(EncodedImage encodedImage, Config config, int i) {
        ...
        inputStream = !isCompleteAt ? new TailAppendingInputStream(limitedInputStream, EOI_TAIL) : limitedInputStream;
        Object obj = decodeOptionsForStream.inPreferredConfig != Config.ARGB_8888 ? 1 : null;
        try {

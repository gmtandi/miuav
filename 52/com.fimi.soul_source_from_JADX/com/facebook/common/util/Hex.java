...
public class Hex {
    ...
    static {
        ...
        FIRST_CHAR = new char[Opcodes.ACC_NATIVE];
        SECOND_CHAR = new char[Opcodes.ACC_NATIVE];
        for (i = 0; i < Opcodes.ACC_NATIVE; i++) {
            FIRST_CHAR[i] = HEX_DIGITS[(i >> 4) & 15];
            SECOND_CHAR[i] = HEX_DIGITS[i & 15];
        }
        DIGITS = new byte[Opcodes.DSUB];
        for (i = 0; i <= 70; i++) {
            DIGITS[i] = (byte) -1;
        }
        for (byte b = (byte) 0; b < 10; b = (byte) (b + 1)) {
            DIGITS[b + 48] = b;
        }
        while (i2 < 6) {
    ...
    public static byte[] hexStringToByteArray(String str) {
        return decodeHex(str.replaceAll(MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR, C2915a.f14760f));
    }
}

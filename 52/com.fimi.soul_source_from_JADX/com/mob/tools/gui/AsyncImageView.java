...
public class AsyncImageView extends ImageView implements Callback, BitmapCallback {
    ...
    private Bitmap goRound(Bitmap bitmap) {
        try {
            int[] size = getSize();
            bitmap = BitmapHelper.roundBitmap(bitmap, size[0], size[MSG_IMG_GOT], this.rect[0], this.rect[MSG_IMG_GOT], this.rect[2], this.rect[3]);
        } catch (Throwable th) {
            MobLog.getInstance().m750w(th);
        }
        return bitmap;
    ...
    public void onImageGot(String str, Bitmap bitmap) {
        ...
        UIHandler.sendMessageDelayed(message, (long) rnd.nextInt(ae.f9482j), this);
    }
